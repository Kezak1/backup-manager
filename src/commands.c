...
{
    for (int i = 0; i < n; i++)
        if (strcmp(b[i].source, src) == 0)
        {
            for (int j = 0; j < b[i].count; j++)
                if (strcmp(b[i].targets[j], target) == 0)
                {
                    return 1;
                }
        }
...
int find_backup_by_source(Backups* state, const char* abs_src)
{
    ...
    {
        if (strcmp(state->backups[i].source, abs_src) == 0)
            return i;
    }
...
void dfs_clean(const char* src, const char* target)
{
    ...
    while ((dp = readdir(dir)) != NULL)
    {
        ...
        if (s1 < 0 || s1 >= (int)sizeof(src_path))
            ERR("snprintf src_path");
        if (s2 < 0 || s2 >= (int)sizeof(target_path))
            ERR("snprintf target_path");
